...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    "*** YOUR CODE HERE ***"
    heuristic = 0
    nvisited = foodGrid.asList()
    p = state[0]
    distances1 = []
    
    if len(nvisited) == 0: return heuristic

    for i in nvisited:
        for j in nvisited:
            d = mazeDistance(i, j, problem.startingGameState)
            distances1.append((d, i, j))
    distances1 = sorted(distances1, reverse = True)

    heuristic += distances1[0][0]
    distance2= []
    distance2.append(mazeDistance(distances1[0][1],p, problem.startingGameState))
    distance2.append(mazeDistance(distances1[0][2],p, problem.startingGameState))
